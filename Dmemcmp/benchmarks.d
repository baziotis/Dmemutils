...
void main(string[] args)
{
    ...
    writeln("size(bytes) Cmemcmp(GB/s) Dmemcmp(GB/s)");
    /*
...
void Cmemcmp(const ref ubyte[] dst, const ref ubyte[] src)
{
    import core.stdc.string: memcmp;
    assert(dst.length == src.length);
    pragma(inline, true)
    memcmp(cast(void*) dst.ptr, cast(void*) src.ptr, dst.length);
}

void Cmemcmp(T)(ref const T dst, ref const T src)
{
    import core.stdc.string: memcmp;
    pragma(inline, true)
    memcmp(cast(void*) &dst, cast(void*) &src, T.sizeof);
}

Duration benchmark(alias f)(ref const ubyte[] dst, ref const ubyte[] src, ulong *bytesCopied)
{
    assert(dst.length == src.length);
...
Duration benchmark(T, alias f)(const ref T dst, const ref T src, ulong *bytesCopied)
{
...
void testStaticType(T)()
{
    ...
    ulong bytesCopied1;
    ulong bytesCopied2;
    immutable d1 = benchmark!(T, Cmemcmp)(d, s, &bytesCopied1);

    immutable d2 = benchmark!(T, Dmemcmp)(d, s, &bytesCopied2);
...
void test(size_t n)()
{
    ...
    int j = 0;
    {
        ...
        foreach(i; 0..alignments)
        {
            ...
            ulong bytesCopied1;
            ulong bytesCopied2;
            immutable d1 = benchmark!(Cmemcmp)(p, q, &bytesCopied1);

            immutable d2 = benchmark!(Dmemcmp)(p, q, &bytesCopied2);
